...
public class Simulateur {
	...
	/** amplitude minimale et maximale du signal analogique */
    ...
    public  Simulateur(String [] args) throws ArgumentsException, InformationNonConformeException {
		...
		// - NRZ et NRZT: signaux min et max, seuil = 0.0
		float seuil;
		if ("RZ".equalsIgnoreCase(form)) {
		    seuil = amplMin + (amplMax - amplMin) / 6.0f;  // pour RZ (même principe que ton 1/6 avant)
		} else {
		    seuil = (amplMin + amplMax) / 2.0f;  // milieu entre les deux niveaux pour NRZ et NRZT
		}
    	...
    	for (int i=0;i<args.length;i++){ // traiter les arguments 1 par 1
    		...
    		if (args[i].matches("-s")){
			    ...
			    try {
			        ...
			        if (amplMin >= amplMax) {
			            throw new ArgumentsException("Valeurs invalides pour -ampl : min doit être < max");
			        }
			    } catch (Exception e) {
			        throw new ArgumentsException("Valeurs invalides pour -ampl : " + e.getMessage());
			    }
			}
