...
public class Emetteur<T> extends Source<Float> implements DestinationInterface <T>{
    ...
    public Emetteur(String typeCodage, int nbEch, float amplMin, float amplMax) {
        ...
        this.nbEch=nbEch;
        this.amplitudeMin = amplMin;
        this.amplitudeMax = amplMax;
    }
            ...
            if ("NRZ".equalsIgnoreCase(typeCodage)) {
                 for (Boolean bit : informationRecue) {
                     for (int i = 0; i < nbEch; i++) {
                         informationGeneree.add(bit ? amplitudeMax : amplitudeMin);
                     }
                }
                ...
                float prevLevel = amplitudeMin; // Niveau précédent (commence à 0)

                for (int b = 0; b < informationRecue.nbElements(); b++) {
                    float level = informationRecue.iemeElement(b) ? amplitudeMax : amplitudeMin;
                    int third = nbEch / 3;
